        ...
        while (1)
        {
            ...
            if (caractere_lu == argv[mot_execute][i])
            {
                ...
                if (caractere_lu == etat_actuel)
                {
                    ...
                    {
                        printf("\nboucle interdite\n");
                        printf("\nla position numero %d est interdite \n", positions_interdites[compteur]);
                        printf("\n position actuelle : %ld", ftell(fichier));

                        if (positions_interdites[compteur] == ftell(fichier))
                        {
                            est_interdite = 1;
                            printf("\non ne prend pas la transition car interdite\n");
                            break;
                        }
                        compteur++;
                    }
                    compteur = 0;
                    printf("\nfin boucle interdite\n");
                }
                ...
                if ((caractere_lu == etat_actuel) && (est_interdite == 0))
                {
                    ...
                    printf("\n nombre de transitions prises : %d", nb_transitions_prises);
                    historique_positions[nb_transitions_prises] = ftell(fichier);
                    printf("\nj'ajoute la position %ld dans l'historique ! \n", historique_positions[nb_transitions_prises]);
                    printf("\non prend la transition\n");
            ...
            else if (caractere_lu == EOF)
            {
                ...
                else // sinon on revient à la transition précédente
                {
                    ...
                    printf("\ncompteur de positions interdites: %d\n", compteur_positions_interdites);
                    printf("\nla position : %ld devient interdite \n", historique_positions[nb_transitions_prises - 1]);
